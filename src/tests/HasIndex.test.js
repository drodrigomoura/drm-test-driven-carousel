...
describe("HasIndex()", () => {
  ...
  it("should has initial `index` state equal to the `defaultIndex` prop", () => {
    expect(wrapper.state("index")).toBe(0);
    const wrapper2 = shallow(<MockComponentWithIndex defaultIndex={1} />);
    expect(wrapper2.state("index")).toBe(1);
  });

  it("should always has `index` state equal to the `index` prop", () => {
  ...
  it("should allow `index` state to change if the `index` prop is unset", () => {
    ...
    wrapperWithInitialIndex.setProps({ index: 3 });
